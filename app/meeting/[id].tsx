...
import { ArrowLeft, MapPin, Clock, Calendar as CalendarIcon } from 'lucide-react-native';
import { fetchSessions, SessionItem } from '@/lib/api/meetings';
    ...
    return (
        <View className="flex-1 bg-background">
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab} className="flex-1">
                ...
                <TabsContent value="sessions" className="flex-1 p-4 bg-muted/20">
                    {loading ? (
                            ...
                            {sessions.map((session, index) => {
                                ...
                                return (
                                    <View
                                        ...
                                        className={`flex-row items-center justify-between py-3 ${index !== sessions.length - 1 ? 'border-b border-border/30' : ''} ${isRace ? 'bg-primary/5 -mx-2 px-2 rounded-lg' : ''}`}
                                    >
                                        <View className="flex-row items-center flex-1 pr-2">
                                            <View className={`w-2.5 h-2.5 rounded-full mr-3 ${isRace ? 'bg-primary' : isQuali ? 'bg-orange-500' : 'bg-muted-foreground/40'}`} />
                                            <Text
                                                className={`text-base flex-1 ${isRace ? 'font-bold text-foreground' : 'font-medium text-foreground'}`}
                                                numberOfLines={1}
                                            >
